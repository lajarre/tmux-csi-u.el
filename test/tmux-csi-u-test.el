;;; tmux-csi-u-test.el --- tests -*- lexical-binding: t; -*-

;;; Commentary:

;; ERT coverage for tmux-csi-u.
  ...
  (let* ((selected 'selected-frame)
    ...
    (cl-letf (((symbol-function 'selected-frame) (lambda () selected))
      ...
      (let ((input-decode-map selected-keymap)
        ...
        (setq report (tmux-csi-u-enable target))))
...
(ert-deftest tmux-csi-u-test-enable-installs-per-terminal-map ()
  (let* ((frame-a 'frame-a)
    ...
    (cl-letf (((symbol-function 'tmux-csi-u--support-state)
      ...
      (let ((input-decode-map selected-keymap)
            (tmux-csi-u-last-report nil))
        (setq report-a (tmux-csi-u-enable frame-a))
        (setq report-b (tmux-csi-u-enable frame-b))))
...
(ert-deftest tmux-csi-u-test-enable-skips-unsupported-without-resolving-terminal-map ()
  (let* ((frame 'unsupported-frame)
    ...
    (cl-letf (((symbol-function 'tmux-csi-u--support-state)
      ...
      (let ((input-decode-map selected-keymap)
            (tmux-csi-u-last-report nil))
        (setq report (tmux-csi-u-enable frame))))
    ...
    (should (= (plist-get report :unsupported-or-skipped)
               (length (tmux-csi-u--candidate-table))))
    (let ((rendered (tmux-csi-u--render-report report)))
...
(ert-deftest tmux-csi-u-test-owned-bindings-cache-uses-weak-keys ()
  (let* ((keymap (make-sparse-keymap))
    ...
    (should (eq (hash-table-weakness tmux-csi-u--owned-bindings-by-keymap)
                'key))
    ...
    (cl-letf (((symbol-function 'selected-frame) (lambda () 'hook-frame))
              ((symbol-function 'tmux-csi-u-enable)
               (lambda (&optional frame)
                 ...
                 '(:status installed))))
      (should (equal (tmux-csi-u--tty-setup-enable)
                     '(:status installed)))
  ...
  (let ((input-decode-map (make-sparse-keymap))
    ...
    (cl-letf (((symbol-function 'tmux-csi-u--support-state)
              ...
              ((symbol-function 'tmux-csi-u--warn-on-new-conflicts)
               (lambda (&rest _args)
                 (ert-fail "Unexpected conflict warning"))))
      (let ((initial-report (tmux-csi-u-enable)))
        ...
        (let ((override-report (tmux-csi-u-enable)))
        ...
        (let ((external-binding (vector 'f14)))
          ...
          (let ((report (tmux-csi-u-enable)))
            ...
            (should (eq (lookup-key input-decode-map sequence)
                        external-binding))
            (should (eq (tmux-csi-u-core--owned-binding
                         (tmux-csi-u--owned-bindings input-decode-map)
                         sequence)
                        tmux-csi-u-core--missing-owned-binding))
            (should (equal report tmux-csi-u-last-report))))))))
...
(ert-deftest tmux-csi-u-test-enable-preserves-truly-external-binding-on-reenable ()
  (let ((input-decode-map (make-sparse-keymap))
    ...
    (cl-letf (((symbol-function 'tmux-csi-u--support-state)
      ...
      (should (eq (plist-get (tmux-csi-u-enable) :status) 'installed))
...
(ert-deftest tmux-csi-u-test-warn-on-new-conflicts-reports-current-preserved-count ()
  (let* ((frame 'frame)
    ...
    (cl-letf (((symbol-function 'frame-terminal)
      ...
      (tmux-csi-u--warn-on-new-conflicts
       frame
    ...
    (should (equal warnings
                   '("2 CSI-u conflicts preserved; inspect (tmux-csi-u-describe) for details")))
    (should (= (length (alist-get param terminal-params)) 2))))

;;; tmux-csi-u-test.el ends here
