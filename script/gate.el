...
(defconst tmux-csi-u-script--allowed-package-lint-warnings nil
  "Package-lint warning messages tolerated for this repo.")

(defun tmux-csi-u-script--tracked-files ()
  "Return Git-tracked files in the repo as absolute paths."
  (mapcar #'tmux-csi-u-script--repo-file
          (process-lines "git" "-C" tmux-csi-u-script--repo-root
                         "ls-files")))

(defun tmux-csi-u-script--repo-file (relative-path)
  "Return RELATIVE-PATH resolved from the repo root."
  (expand-file-name relative-path tmux-csi-u-script--repo-root))

(defun tmux-csi-u-script--relative-path (path)
  "Return PATH relative to the repo root."
  (file-relative-name path tmux-csi-u-script--repo-root))

(defun tmux-csi-u-script--extension-in-p (path extensions)
  "Return non-nil when PATH has an extension in EXTENSIONS."
  (member (downcase (or (file-name-extension path) "")) extensions))

(defun tmux-csi-u-script--format-files ()
  "Return repo files covered by the canonical formatter."
  (sort
   (delete-dups
    (cl-remove-if-not
     (lambda (path)
       (tmux-csi-u-script--extension-in-p
        path '("el" "json" "md" "yml" "yaml")))
     (tmux-csi-u-script--tracked-files)))
   #'string<))

(defun tmux-csi-u-script--lint-files ()
...
(defun tmux-csi-u-script--compile-files ()
  "Return Elisp files covered by byte compilation."
  (cl-remove-if-not
   (lambda (path)
     (and (string= (file-name-directory path)
                   tmux-csi-u-script--repo-root)
          (string= (downcase (or (file-name-extension path) "")) "el")))
   (tmux-csi-u-script--tracked-files)))

(defun tmux-csi-u-script--normalize-current-buffer (path)
...
(defun tmux-csi-u-script--format-file (path check-only)
...
When CHECK-ONLY is non-nil, do not write the formatted contents."
  (with-temp-buffer
    ...
    (let ((before (buffer-string)))
      (tmux-csi-u-script--normalize-current-buffer path)
...
(defun tmux-csi-u-script--run-format (check-only)
  "Run the formatter with CHECK-ONLY semantics."
  (let (changed)
    (dolist (path (tmux-csi-u-script--format-files))
      (when (tmux-csi-u-script--format-file path check-only)
        (push (tmux-csi-u-script--relative-path path) changed)))
...
(defun tmux-csi-u-script-format ()
  "Apply canonical formatting to repo files in scope."
  (tmux-csi-u-script--run-format nil))

(defun tmux-csi-u-script-format-check ()
  "Fail when repo files are not in canonical format."
  (tmux-csi-u-script--run-format t))

(defun tmux-csi-u-script--checkdoc-output (path)
  "Return `checkdoc' output for PATH, or nil when PATH is clean."
  (let ((diagnostic-buffer " *tmux-csi-u-checkdoc*"))
...
(defun tmux-csi-u-script--activate-package-archives ()
  "Initialize repo-local package archives for gate helpers."
  (require 'package)
  (setq package-user-dir (tmux-csi-u-script--repo-file ".tmp/elpa"))
  (setq package-archives '(("gnu" . "https://elpa.gnu.org/packages/")
                           ("nongnu" . "https://elpa.nongnu.org/nongnu/")))
  (package-initialize)
  t)

(defun tmux-csi-u-script-bootstrap-package-lint ()
  "Install `package-lint' into the repo-local package dir."
  (tmux-csi-u-script--activate-package-archives)
...
(defun tmux-csi-u-script--package-lint-issues ()
  ...
  (with-current-buffer (find-file-noselect
                        (tmux-csi-u-script--repo-file "tmux-csi-u.el"))
    (package-lint-buffer)))

(defun tmux-csi-u-script--print-package-lint-issues (issues)
...
(defun tmux-csi-u-script--package-lint-warning-allowed-p (issue)
  "Return non-nil when package-lint ISSUE is explicitly allowed."
  (member (nth 3 issue)
          tmux-csi-u-script--allowed-package-lint-warnings))

(defun tmux-csi-u-script-lint ()
  "Run repo lint gates."
  (let (checkdoc-failures)
    (dolist (path (tmux-csi-u-script--lint-files))
      (when-let ((output (tmux-csi-u-script--checkdoc-output path)))
    ...
    (when checkdoc-failures
      ...
      (dolist (failure checkdoc-failures)
        (princ (format "[%s]\n%s\n"
                       (tmux-csi-u-script--relative-path (car failure))
                       (cdr failure)))))
    (let* ((issues (tmux-csi-u-script--package-lint-issues))
           (warnings (cl-remove-if-not (lambda (issue)
                                         (eq (nth 2 issue) 'warning))
                                       issues))
           (allowed-warnings (cl-remove-if-not
                              #'tmux-csi-u-script--package-lint-warning-allowed-p
                              warnings))
      ...
      (when allowed-warnings
        ...
        (tmux-csi-u-script--print-package-lint-issues allowed-warnings))
      (when unexpected-warnings
        (princ "package-lint warnings:\n")
        (tmux-csi-u-script--print-package-lint-issues unexpected-warnings))
      (when errors
        (princ "package-lint errors:\n")
        (tmux-csi-u-script--print-package-lint-issues errors))
      (if (or checkdoc-failures unexpected-warnings errors)
          (kill-emacs 1)
        (princ "lint ok\n")))))

(defvar tmux-csi-u-script--compile-output-dir nil
...
(defun tmux-csi-u-script--compile-file-or-die (path)
  ...
  (when-let ((buffer (get-buffer byte-compile-log-buffer)))
    (let ((output (with-current-buffer buffer
      ...
      (unless (string-empty-p output)
        (princ (format "[%s]\n%s\n"
                       (tmux-csi-u-script--relative-path path)
                       output))
        (kill-emacs 1)))))

(defun tmux-csi-u-script-compile ()
  "Byte-compile package files with warnings treated as failures."
  (let* ((tmux-csi-u-script--compile-output-dir
          (tmux-csi-u-script--repo-file ".tmp/elc"))
         (byte-compile-error-on-warn t)
         (load-prefer-newer t)
         (byte-compile-dest-file-function
          #'tmux-csi-u-script--compile-dest-file))
